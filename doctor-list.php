...
$alert = false;
    ...
    if (isset($_GET['deleteimg'])) {
        ...
        $sno = $_GET['deleteimg'];
        $sql = "UPDATE doctor SET photo = null WHERE `id` = '$sno'";
        $result = mysqli_query($conn, $sql);
        if ($result) {
            $success = "Deleted successfully.";
        }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['snoEdit'])) {
        ...
        $sql = "UPDATE doctor set `name` = '$name', `title` = '$title', `email` = '$email', `address` = '$address', `phone` = '$phone', `about` = '$about' where `doctor`.`id` = '$id'";
        $result = mysqli_query($conn, $sql);
        if ($result) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add']) ) {
        ...
        $sql = "INSERT INTO doctor set `name` = '$name', `title` = '$title', `email` = '$email', `address` = '$address', `phone` = '$phone', `about` = '$about'";
        $result = mysqli_query($conn, $sql);
    ...
    if(isset($_POST['addphoto']) && !empty($_FILES["img"]["name"]) ){
        ...
        if(in_array($fileType, $allowTypes)){
            ...
            $sql =  "UPDATE `doctor` SET `photo` = '$imgContent' WHERE `doctor`.`id` = '$id'";
...
<div class="modal fade mx-auto" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form class=" mx-auto p-5 " action="doctor-list.php" method="post">
...
<div class="modal fade mx-auto" id="addModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content ">
            ...
            <form class=" mx-auto p-5 " action="doctor-list.php" method="post">
                <h1 class="text-center pb-4">Add New Doctor</h1>
...
<div class="modal fade mx-auto" id="imgmodal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form class=" mx-auto p-5" action="doctor-list.php" method="post" enctype="multipart/form-data">
...
<div class="container overflow-auto">
    <h1 class="text-center mt-3">Doctor List</h1>
    <button id="add-doctor" class="float-end mb-5 py-2 btn btn-sm btn-outline-primary"> <i
            class="bi bi-person-plus-fill pe-2"></i>Add New Doctor</button>

    <table id="table" class="table table-bordered table-hover table-sm text-sm">
        <thead class="table-dark">
            <tr>
                <th>ID</th>
                <th class="text-break">Doctor Name</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <?php
                    if(!empty($row['photo'])){?>
                        <img class="py-2" style="width: 10rem;" src='data:image/jpg;charset=utf8;base64, <?php echo base64_encode($row['photo']);?>' />
                    <?php } ?>
...
Array.from(deletes).forEach((element) => {
    element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to Delete this!")) {
            window.location = `doctor-list.php?delete=${sno}`;
        }
    })
})
...
Array.from(deleteimgs).forEach((element) => {
    element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to Delete this!")) {
            window.location = `doctor-list.php?deleteimg=${sno}`;
        }
    })
})
